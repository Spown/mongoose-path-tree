...
function tree(schema, options) {
    ...
    var pathSeparator = options && options.pathSeparator || '#'
        ...
        , idType = (options && options.idType) || schema.constructor.ObjectId
    ...
    schema.pre('save', function preSave(next) {
        ...
        if (this.isNew || isParentChange) {
            ...
            this.collection.findOne({ _id: this.parent }, function (err, doc) {
                ...
                self.path = (doc ? (doc.path + pathSeparator) : '') + self._id.toString();
        ...
        if (onDelete === 'DELETE' && this.path) {
            ...
            this.collection.find({ parent: previousParent }, function (err, cursor) {
                ...
                function streamOnClose(err) {
                    ...
                    previousParent && self.collection.find({ path: { $regex: previousParent + pathSeparatorRegex} }, function (err, cursor) {
    ...
    schema.statics.getChildrenTree = function getChildrenTree(root, args, next) {
        ...
        function produceTree(err, results) {
            ...
            results.forEach && results.forEach(function(r){
                createChildren(finalResults, r, getLevel(r.path));
            })

            return finalResults;
        }
        
        return this.find(filters, fields).then(function (results) {
            var tree = produceTree(null, results)
            next && next(undefined, tree);
            return tree;
        }, function(err) {
            next && next(err);
            return err;
