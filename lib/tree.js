...
function tree(schema, options) {
    ...
    var pathSeparator = options && options.pathSeparator || '#',
        ...
        treeOrdering = options && options.treeOrdering || false,
            pFN = treeOrdering===true ? 'position' : String(treeOrdering)
    ;
    ...
    if (treeOrdering) {
        properties[treeOrdering === true ? 'position' : pFN] = {
            type: Number
        };
    }
    ...
    schema.methods.getSiblingsAndSelf = function(next) {
        var query = this.model(this.constructor.modelName).find({parent: this.parent});
        if (treeOrdering) {
    ...
    schema.methods.moveToPosition = function(newPosition, next) {
        if (!treeOrdering || (newPosition === this[pFN])) {next(null, this);return;}
        var self = this,
            query = this.model(this.constructor.modelName).where({ parent: this.parent }),
            options = { multi: true },
            posDir = -1
        ;
        function updatePosition(err) {
            if (err) {next(err);return;}
            self[pFN] = newPosition;
            self.save(next);
        }
        if (newPosition > this[pFN]) {
            query = query.where({ position: { $gt: this[pFN], $lte: newPosition } });
        } else {
            query = query.where({ position: { $gte: newPosition, $lt: this[pFN] } });
            posDir = 1;
        }
        return query.setOptions(options).update({ $inc: { position: posDir } }, updatePosition);
    };
