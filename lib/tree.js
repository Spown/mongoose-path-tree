...
function tree(schema, options) {
    ...
    schema.pre('save', function preSave(next) {
        ...
        function assignDefaultPosition(cb) {
            if (treeOrdering && self.position === undefined || isParentChange) {
    ...
    schema.methods.moveToPosition = function(newPosition, next) {
        if (newPosition === this.position) next(null, this);
        var self = this;
        ...
        var query = this.model(this.constructor.modelName).where({ parent: this.parent });
        var options = { multi: true };
        if (newPosition > this.position) {
            query.where({ position: { $gt: this.position, $lte: newPosition } })
                 .setOptions(options)
                 .update({ $inc: { position: -1 } }, updatePosition);
        } else {
            query.where({ position: { $gte: newPosition, $lt: this.position } })
                 .setOptions(options)
                 .update({ $inc: { position: 1 } }, updatePosition);
        }
    }
