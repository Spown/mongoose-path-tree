...
function tree(schema, options) {
    ...
    schema.statics.getChildrenTree = function getChildrenTree() { //root, args, next
        ...
        var filters = args.filters || {},
            fields = args.fields || (args.fields === null ? null : (args.fields === undefined ? null : args.fields)),
        ...
        if (fields instanceof Object) {
            if (!fields.hasOwnProperty('path') || !fields.path) {
                fields.path = 1;
            }
            if (!fields.hasOwnProperty('parent') || !fields.parent) {
                fields.parent = 1;
            }
            if (treeOrdering && (!fields.hasOwnProperty(pFN) || !fields[pFN]) ) {
                fields[pFN] = 1;
            }
            ...
            if (!fields.match(/\+?path/) ) {
                ...
                fields = fields.replace(/-(path)/, '$1');
            }
            if (!fields.match(/\+?parent/)) {
                fields += ' parent';
            } else {
                fields = fields.replace(/-(parent)/, '$1');
            }
            if (treeOrdering) {
                if (!fields.match(RegExp('\\+?' + escapeRegExp(pFN)))) {
                    fields += ' ' + pFN;
                } else {
                    fields = fields.replace(RegExp('-(' + escapeRegExp(pFN) + ')'), '$1');
                }
            }
        }
